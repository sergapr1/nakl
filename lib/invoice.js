...
function extractInvoiceFromText(text, invoiceId) {
  const parts = text
    .replace(/[;,]/g, "\n")
    .split("\n")
    .map(s => s.trim())
...
function parseEta(text) {
  ...
  if (m) return { y:+m[3], mo:+m[2], d:+m[1], hh:+m[4], mm:+m[5] };

  m = t.match(/^(\d{2})\.(\d{2})\s+(\d{2}):(\d{2})$/);
  if (m) {
    const now = new Date();
    return { y: now.getFullYear(), mo:+m[2], d:+m[1], hh:+m[3], mm:+m[4] };
  }
...
module.exports = { todayISO, calc, extractInvoiceFromText, formatInvoice, parseEta, applyDeliveryCommand };
